...
async def modify_permission(permission_id: int, user_id: int, permission: PermissionCreate, db: Session = Depends(get_db)):
    ...
    if db_permission:
        ...
        return JSONResponse(content={"message": "Permission modified or created successfully", "permission": db_permission_dict}, status_code=200)
    else:
...
async def assign_user_plan(userid: int, subscription: Subscriptionpost, db: Session = Depends(get_db)):
    ...
    if db_subscription:
        ...
        db_subscription.user_id = userid
...
async def view_usage_statistics(user_id: int, db: Session = Depends(get_db)):
    ...
    if user_plan and 'a' in p.permissions:
        ...
        if user_usage.api_endpoint > 0:
            ...
            return JSONResponse(content={"access": "Premium API endpoint Granted /n ONLY PERMISSION TO READ"}, status_code=200)
        else:
