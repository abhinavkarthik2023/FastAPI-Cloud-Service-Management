...
async def create_plan(plan: PlanCreate,token:str, db: Session = Depends(get_db)):
    ...
    # existing_user = db.query(User).filter(User.id == plan.user_id).first()
    # if not existing_user:
    #     raise HTTPException(status_code=404, detail="User does not exist in the User table")
    
    que =  db.query(Permission).filter(plan.permissions==Permission.name)
    if que:
        raise HTTPException(status_code=403, detail="Permission does not exists")
...
# @app.post("/permissions/{user_id}", response_model=PermissionCreate)
# async def add_permission(user_id:int, permission: PermissionCreate, db: Session = Depends(get_db)):
...
@app.put("/permissions/{permission_id}/{user_id}", response_model=PermissionCreate)
async def modify_permission(permission_id: int,user_id:int, permission: PermissionCreate, db: Session = Depends(get_db)):
    status_check = db.query(User).filter(User.id==user_id, User.status == "user").first()
    if status_check:
        raise HTTPException(status_code=404, detail="Accesss Denied")
...
async def assign_user_plan(userid: int, subscription: Subscriptionpost, db: Session = Depends(get_db)):
        ...
        status_check = db.query(User).filter(subscription.user_id==1, User.status == "admin").first()
        if status_check:
            raise HTTPException(status_code=404, detail="admin cannot subscribe")

        status_check = db.query(User).filter(User.id == userid, User.status == "admin").first()
        if not status_check:
            raise HTTPException(status_code=404, detail="user don't have admin status")
        ...
        db_usage = Usage(user_id=subscription.user_id, api_endpoint=usage_limit, timestamp=usage_limit)
        db.add(db_usage)
...
async def view_usage_statistics(user_id: int, db: Session = Depends(get_db)):
    ...
    if user_plan and 'b' in p.permissions:
        ...
        if user_usage.api_endpoint > 0:
            ...
            db.commit()
            # Return the updated usage record
            return JSONResponse(content={"access": "Basic API endpoint Granted"}, status_code=200)
        else:
            return JSONResponse(content={"access": "Blocked - Usage limit reached"}, status_code=200)
    else:
...
async def view_usage_statistics(user_id: int, db: Session = Depends(get_db)):
    ...
    if user_plan and 'c' in p.permissions:
        ...
        if user_usage.api_endpoint > 0:
            ...
            db.commit()
            # Return the updated usage record
            return JSONResponse(content={"access": "Standard API endpoint Granted"}, status_code=200)
        else:
            return JSONResponse(content={"access": "Blocked - Usage limit reached"}, status_code=200)
    else:
